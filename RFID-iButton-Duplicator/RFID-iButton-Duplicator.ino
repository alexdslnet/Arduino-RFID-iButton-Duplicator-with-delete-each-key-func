...
emkeyType keyType;
enum emMode {md_empty, md_read, md_write, md_blue, md_threeMode};               // режим работы копировальщика
...
bool write2Mifare() {
  ...
  if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
    ...
    for (byte i = 0; i < rfid.uid.size; i++) {
      addr[i] = rfid.uid.uidByte[i];
      if (addr[i] != keyID[i]) {
        Check = true;  // сравниваем код для записи с тем, что уже записано в ключе.
        break;
      }
    }
...
bool EEPROM_DeleteKey(byte buf[]) {
 
  byte indx = indxKeyInROM(buf); // Find the key in EEPROM

  if (indx == 0) {
    Serial.println(F("Key not found in EEPROM."));
    OLED_printError(F("Key not found"));
    Sd_ErrorBeep();
    return false;
  }

  Serial.println(F("Deleting key and shifting remaining keys in EEPROM"));
  
  // Shift all keys after the deleted one
  for (byte i = indx; i < EEPROM_key_count; i++)
  {
    byte nextKey[8];
    EEPROM.get((i + 1) * sizeof(nextKey), nextKey);
    EEPROM.put(i * sizeof(nextKey), nextKey);
  }

  // Clear the last key slot
  byte emptyKey[8] = {0};
  EEPROM.put(EEPROM_key_count * sizeof(emptyKey), emptyKey);

  // Update EEPROM key count
  EEPROM_key_count--;
  EEPROM.update(0, EEPROM_key_count);

  Serial.println(F("Key deleted and keys shifted successfully"));
...
void loop() {
  ...
  if ((echo == 't') || enc1.isClick()) {  // переключаель режима чтение/запись
    switch (copierMode) {
      ...
      case md_read: copierMode = md_write; clearLed(); digitalWrite(R_Led, HIGH);
        delayMicroseconds(5); break;
      case md_write: copierMode = md_threeMode; clearLed(); digitalWrite(B_Led, HIGH);
        delayMicroseconds(5); break;
      case md_threeMode: copierMode = md_blue; clearLed(); digitalWrite(B_Led, HIGH); digitalWrite(G_Led, HIGH);
        delayMicroseconds(5); break;
      case md_blue: copierMode = md_read; clearLed(); digitalWrite(G_Led, HIGH);
        delayMicroseconds(5); break;
    }
    OLED_printKey(keyID);
